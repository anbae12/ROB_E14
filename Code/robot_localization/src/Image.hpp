...
public:
    ...
    void setPixel(size_t x, size_t y, const Pixel4i& value);
    ...
    void paintBorder( const Pixel4i& color){
        for(int x=0;x<getWidth();x++){
            setPixel(x,0, color);
            setPixel(x,getHeight()-1, color);
        }
        for(int y=0;y<getHeight();y++){
            setPixel(0, y, color);
            setPixel(getWidth()-1,y, color);
        }
    }

    /**
     * @brief copy image to dst
     * @param dst
     */
    void copyTo(Image* dst){
        for(int x=0;x<getWidth();x++){
            for(int y=0;y<getHeight();y++){
                dst->setPixel(x,y, getPixelf(x,y) );
            }
        }
    }
