...
void Image::setPixel(size_t x, size_t y, const Pixel4i& value) {
    const size_t idx = y*_widthStep + x*_nrChannels;
    ...
    // now if representation is float then we can set it directly
    if(_depth == Image::Depth32F){
        ((float*)&_imageData[cidx])[0] = _toFloat*value.ch[0];
        for(size_t i=1;i<_nrChannels;i++)
            ((float*)&_imageData[cidx])[i] = _toFloat*value.ch[i];
    } else if(_depth == Image::Depth32S) {
        for(size_t i=0;i<_nrChannels;i++)
            ((int32_t*)&_imageData[cidx])[i] = (int32_t)(value.ch[i]);
    } else if( _depth == Image::Depth16S || _depth == Image::Depth16U) {
        for(size_t i=0;i<_nrChannels;i++)
...
{
    if(_colorCode!=GRAY){
        THROW("Image must be of type GRAY!");
    }
...
/*
    ...
    */
    ...
    if (getBitsPerPixel() == 8) {
        ...
        fprintf(imagefile,"# CREATOR: RobWork - www.robwork.dk \n");
        fprintf(imagefile,"%u %u \n", _width, _height);
        fprintf(imagefile,"65530\n");

        // now print all row in reverse order
        for(size_t y=0;y<_height;y++){
            size_t idx = y*_widthStep;
            for(size_t x=0;x<_width;x++){
                uint16_t *arr = (uint16_t*) _imageData;
                fprintf(imagefile,"%u ", arr[idx+x]);
            }
            fprintf(imagefile,"\n");
        }
...
bool Image::saveAsPPM(const std::string& fileName) const
{
    ...
    if (imagefile == NULL) {
        THROW("Can't open image file for writing!");
        return false;
    }
    ...
    if (getBitsPerPixel() == 8) {
        ...
        fclose(imagefile);
        THROW("Image depth of " << getBitsPerPixel() << " is not supported!");
        return false;
    }
