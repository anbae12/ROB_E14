...
//  laser_range_scanner
//
//  Created by Jannik Gade on 03/12/14.
//
//
...
LinearRegression::LinearRegression(std::vector<pixel> p)
{
    long i;
    a = b = sumX = sumY = sumXsquared = sumYsquared = sumXY = 0.0;
    n = 0L;
    ...
    if (p.size() > 0L) // if size greater than zero there are data arrays
        for (n = 0, i = 0L; i < p.size(); i++){
            temp_point.setX(a[i].get_x());
            temp_point.setY(a[i].get_y());
            addPoint(temp_point);
        }
